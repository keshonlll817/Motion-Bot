...
# ── ROLE IDs ──
TT_OFFICIAL_ROLE_ID  = 1428404827234504735   # regular TT plays
...
def build_reminder_text(guild, league, p1, p2, wins, total, tier, label, play_type="", game_dt=None):
    """
    Build the reminder message with the correct role ping.
    12:00 AM–5:59 AM EST → TT Degen, otherwise TT Official.
    Format: @ROLE LEAGUE – P1 vs P2 [OVER/UNDER XU] EMOJI (wins/total) | LABEL
    """
    ...
    role, role_name = _pick_role(guild, game_dt)
    if role:
...
def _allowed_mentions_for_guild(guild, game_dt=None):
    """
    Return AllowedMentions for the correct role based on game time.
    Forces Discord to ping even when the role is NOT publicly mentionable.
    """
    role, _ = _pick_role(guild, game_dt)
    if role:
...
async def _reminder_task(guild_id, key, guild, league, p1, p2, wins, total, tier, game_dt, reminder_channel_id, play_type="", silent_channel_id=None):
    """
    ...
    try:
        ...
        async def _send_alert(label_text):
            """Send to reminder channel (with ping) + main chat (silent)."""
            ch_ping   = await _fetch_ch(reminder_channel_id)
            ch_silent = await _fetch_ch(silent_channel_id) if silent_channel_id else None

            if not (not locked or guild_id == TEST_GUILD_ID):
                print(f"[REMINDERS] {label_text} suppressed (locked): {key}")
                return

            if ch_ping:
                dest_guild = getattr(ch_ping, "guild", guild)
                text = build_reminder_text(dest_guild, league, p1, p2, wins, total, tier, label_text, play_type, game_dt)
                await ch_ping.send(text, allowed_mentions=_allowed_mentions_for_guild(dest_guild, game_dt))
                print(f"[REMINDERS] Sent {label_text}: {key} → ch={reminder_channel_id}")

            if ch_silent:
                dest_guild = getattr(ch_silent, "guild", guild)
                silent_text = build_reminder_text(dest_guild, league, p1, p2, wins, total, tier, label_text, play_type, game_dt)
                # Strip the role mention for the silent copy
                silent_text = re.sub(r'<@&\d+>\s*', '', silent_text).strip()
                await ch_silent.send(silent_text, allowed_mentions=discord.AllowedMentions.none())
                print(f"[REMINDERS] Sent {label_text} (silent): {key} → ch={silent_channel_id}")

        if pre_delay > 0:
            print(f"[REMINDERS] Sleeping {pre_delay:.0f}s for SOON: {key}")
            await asyncio.sleep(pre_delay)
            await _send_alert("STARTING SOON")
        ...
        if start_delay > 0:
            ...
            await _send_alert("STARTING NOW")
...
def _schedule_play_for_message(guild_id, message_id, guild, play, game_dt, reminder_channel_id, silent_channel_id=None):
    ...
    task = asyncio.ensure_future(
        _reminder_task(guild_id, key, guild, league, p1, p2, wins, total, tier, game_dt, reminder_channel_id, play_type, silent_channel_id)
    )
...
async def schedule_message_plays(message, text=None):
    ...
    for line, time_str, game_dt in resolved:
        ...
        # Route reminder channels based on which server/channel the slate is in
        if message.channel.id == SLATE_CHANNEL:
            # Test server
            reminder_channel_id = REMINDERS_CHANNEL
            silent_channel_id   = None
        else:
            # Main server — ping in #train-reminders, silent copy in #main-chat
            reminder_channel_id = MAIN_REMINDER_CHANNEL
            silent_channel_id   = MAIN_CHAT_CHANNEL

        key, was_scheduled = _schedule_play_for_message(
            guild_id, message_id, guild, play, game_dt, reminder_channel_id, silent_channel_id
        )
...
async def send_reminder_confirmation(results, override_channel=None):
    ...
    # Route confirmation based on which server the override_channel is from
    if override_channel and override_channel.id in (SLATE_CHANNEL, FOUR_PLUS_CHANNEL, TOTALS_CHANNEL):
        if override_channel.id == SLATE_CHANNEL:
            ch = client.get_channel(REMINDERS_CHANNEL)
        else:
            ch = client.get_channel(MAIN_CONFIRM_CHANNEL)
    else:
        ch = client.get_channel(MAIN_CONFIRM_CHANNEL)
    if not ch:
        return
...
async def on_ready():
    ...
    # Reschedule from test server slate channel
    slate_ch = client.get_channel(SLATE_CHANNEL)
    if slate_ch:
        await reschedule_from_channel(slate_ch)
        print("[REMINDERS] Rescheduled from test slate channel.")

    # Reschedule from main server 4+ and totals channels
    ...
    if totals_ch:
        ...
        print("[REMINDERS] Rescheduled from main totals channel.")
